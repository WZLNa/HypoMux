"""
NetBooster 代理后端模块 - Phase 4A（SOCKS5 + HTTP 双协议无感接管）
...
class ProxyWorker(QThread):
    ...
    async def _serve(self):
        ...
        self.log_signal.emit(
            f"🚀 SOCKS5+HTTP 分发引擎已启动 | SOCKS {self._listen_host}:{self._listen_port} "
        ...
        try:
            ...
            await self._aggressive_teardown()
            self.log_signal.emit("收到停止指令，已强制关闭监听并销毁所有在途连接")
