...
def create_main_window():
    ...
    class NetworkAdapterTableWidget(QTableWidget):
        ...
        def init_ui(self):
            ...
            self.setHorizontalHeaderLabels([
                "选择", "网卡别名", "IPv4 地址", "实时速度 (MB/s)", "实时连接数"
            ])
    ...
    class MainWindow(QMainWindow):
        ...
        def __init__(self):
            ...
            super().__init__()
            self.setWindowTitle("NetBooster - Windows 多网卡双协议分流加速")
        ...
        def init_ui(self):
            ...
            subtitle_label = QLabel("多网卡 HTTP/HTTPS/SOCKS 分流引擎 · 实时分流大屏")
            ...
            speed_caption = QLabel("合并下行总速度 (MB/s)")
            speed_caption.setStyleSheet("color: #616161; font-weight: 600;")

            speed_box = QVBoxLayout()
            speed_box.setSpacing(2)
            speed_box.addWidget(self.speed_value_label)
            speed_box.addWidget(speed_caption)

            self.up_value_label = QLabel("上行 0.00 MB/s")
            self.up_value_label.setStyleSheet("color: #616161; font-weight: 600;")
            self.conn_value_label = QLabel("总连接数 0")
            ...
            # ---------- 实时控制台 ----------
            console_caption = QLabel("调度控制台")
            console_caption.setStyleSheet("color: #526579; font-weight: 600;")
            shell_layout.addWidget(console_caption)
            ...
            port_label = QLabel("SOCKS 端口")
            port_label.setStyleSheet("color: #526579; font-weight: 600;")
            ...
            self.boost_btn = PrimaryPushButton("一键加速")
            ...
            action_layout.addWidget(port_label)
        ...
        def load_adapters(self):
            ...
            if self._is_boosting:
                self.show_warning("加速运行中，请先停止再刷新网卡")
                return
        ...
        def _start_proxy(self):
            ...
            if not selected:
                self.show_warning("请先勾选至少一张拥有有效 IPv4 的网卡")
                return
            ...
            try:
                ...
                self.append_log(
                    f"[启动] 准备启动双协议分流引擎，SOCKS {self._pending_socks_addr}，"
                    f"HTTP/HTTPS {self._pending_http_addr}，参与网卡: {nic_names}"
                )
                self.status_label.setText("状态: 正在启动双协议分流引擎...")
                ...
                self.append_log(f"[启动] 分流引擎启动异常: {e}")
                self.show_error(f"分流引擎启动失败:\n\n{e}")
        ...
        def _stop_proxy(self):
            try:
                if self.proxy_worker is None:
                    ...
                    self._exit_boosting_ui()
                    return
                self.append_log("[停止] 已发送安全停止指令，正在关闭监听并清理在途连接...")
                ...
                set_system_proxy(False)
                self.append_log("[系统代理] 已强制关闭 Windows 全局代理")

        def _enter_boosting_ui(self):
            self.boost_btn.setText("停止加速")
        ...
        def _exit_boosting_ui(self):
            ...
            self.up_value_label.setText("上行 0.00 MB/s")
            self.conn_value_label.setText("总连接数 0")
        ...
        def on_proxy_traffic(self, payload: dict):
            ...
            self.speed_value_label.setText(f"{total.get('down_mbps', 0.0):.2f}")
            self.up_value_label.setText(f"上行 {total.get('up_mbps', 0.0):.2f} MB/s")
            self.conn_value_label.setText(f"总连接数 {total.get('connections', 0)}")
            self.status_label.setText(
                f"状态: 分流运行中 · 下行 {total.get('down_mbps', 0.0):.2f} MB/s · "
                f"连接 {total.get('connections', 0)}"
            )
            # 各网卡实时速度和连接数回填到表格
        ...
        def on_proxy_started(self, endpoint: str):
            try:
                ...
                self.append_log(
                    "[系统代理] 已接管 Windows 全局代理: "
                    f"http={self._pending_http_addr};https={self._pending_http_addr};"
                    f"socks={self._pending_socks_addr}"
                )
            except Exception as e:
                self.append_log(f"[系统代理] 启用失败，正在停止引擎: {e}")
                self.show_error(f"双协议引擎已监听，但无法写入 Windows 系统代理:\n\n{e}")
                if self.proxy_worker is not None:
                    self.proxy_worker.stop()
                return
            ...
            self._is_boosting = True
            self.boost_btn.setEnabled(True)
            self.status_label.setText(f"状态: 双协议分流引擎运行中 @ {endpoint}")
            self.show_success(
                f"已接管系统代理 · HTTP/HTTPS {self._pending_http_addr} · SOCKS {self._pending_socks_addr}"
            )
        ...
        def on_proxy_error(self, message: str):
            ...
            try:
                set_system_proxy(False)
                self.append_log("[系统代理] 启动失败，已强制关闭 Windows 全局代理")
            except Exception as e:
                self.append_log(f"[系统代理] 启动失败后的清理异常: {e}")
            self.show_error(message)
            # 启动失败后内核会走 stopped 流程收尾，这里只提示

        @Slot(str)
        def on_proxy_stopped(self, message: str):
            self.append_log(f"[已停止] {message}")
            try:
                set_system_proxy(False)
            except Exception as e:
                self.append_log(f"[系统代理] 停止后的清理异常: {e}")
            self._is_boosting = False
            self.status_label.setText("状态: 已停止，就绪")
            self._exit_boosting_ui()
        ...
        def closeEvent(self, event):
            ...
            try:
                ...
                print(f"[ERROR] 退出清理异常: {e}")
            finally:
                try:
                    set_system_proxy(False)
                except Exception as e:
                    print(f"[ERROR] 系统代理关闭异常: {e}")
