...
$Shortcut.Description = "Steam with NetBooster Multi-NIC Acceleration"
$Shortcut.Save()
"""
            ...
            if result.returncode == 0:
                messages.append("✓ 已在桌面创建快捷方式: Steam (NetBooster 加速).lnk")
            else:
        ...
        success_msg += f"2. 确保 NetBooster 已启动加速（端口 {self.proxy_port}）\n"

        if registry_success:
            success_msg += f"3. 正常启动 Steam 即可（无需使用快捷方式）\n"
            success_msg += f"4. 在 Steam 设置 → 下载 中选择合适的下载服务器\n"
        else:
            success_msg += f"3. 使用桌面的「Steam (NetBooster 加速)」快捷方式启动\n"
